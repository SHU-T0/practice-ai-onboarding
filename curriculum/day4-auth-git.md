...
| 9:45-10:30 | Supabase Authでメール/パスワードログイン実装（デモ→全員で実装） | 全体→個人 |
| 10:30-10:45 | 休憩 | - |
| 10:45-12:00 | ログイン付きメモアプリを完成（認証+メモのCRUD） | 個人 |
| 12:00-13:00 | 昼休み | - |
| 13:00-14:30 | メモアプリの機能拡充: メモ追加・編集・削除を実装 | 個人 |
| 14:30-14:45 | 休憩 | - |
...
> **Day 1-3との違い**: これまでは `App.tsx` という1つのファイルでアプリを作ってきました。Day 4からは `app/` フォルダの中にファイルを置く方式（Expo Router）に切り替えます。これは「複数の画面があるアプリ」を作るための仕組みです。最初は戸惑うかもしれませんが、AIに任せれば大丈夫です。
